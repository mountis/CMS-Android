...
public final class CreateTeamDialog extends AlertDialog.Builder {
    public CreateTeamDialog(final View containingView) {
    ...
    private void setupPositiveButton(final ArrayList<TextInputEditText> editTexts) {
                    ...
                    FirebaseHelper.getReference(getContext(),R.string.firebase_teams_directory).addListenerForSingleValueEvent(new ValueEventListener() {
                        ...
                        public void onDataChange(DataSnapshot dataSnapshot) {
                            if (dataSnapshot.hasChildren()) {
                                ...
                                // Create Team
                                createTeamAndAddUserToTeam(editTexts);
                            } else {
                                createTeamAndAddUserToTeam(editTexts);
                            }
                        }
    ...
    private void createTeamAndAddUserToTeam(final ArrayList<TextInputEditText> editTexts) {
        // Get Full User Data
                            ...
                            if (dataSnapshot.hasChildren()) {
                                ...
                                for (DataSnapshot teamReference:dataSnapshot.getChildren()) {
                                    ...
                                    if (teamItem.getType().equals(FirebaseEntity.EntityType.US)) {
                                        ...
                                        save(getContext(),connectedChat);
                                        // Create Channels
                                        createChannels(teamItem, newTeam, connectedChat);
                                        createElements(teamItem);

                                        save(getContext(),newTeam);
    ...
    private void createElements(Team teamItem) {
        for (int i = 0; i < BrandingElement.ElementType.getNumberOfElementTypes(); i++) {
            ...
            save(getContext(),elementItem);
        }
    }

    private void createChannels(Team teamItem, Team newTeam, Chat connectedChat) {
        for (int i = 0; i < 3; i++) {
